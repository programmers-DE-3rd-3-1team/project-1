...
# 시간에 따른 상관계수 계산 (롤링 윈도우 사용)
window_size = 60  # 예: 30일 윈도우
rolling_corr = gold_kospi['kospi_price'].rolling(window=window_size).corr(gold_kospi['gold_price'])

# 롤링 상관계수 그래프
fig = px.line(x=gold_kospi['date'], y=rolling_corr, title='Rolling Correlation: KOSPI vs GOLD')
fig.update_xaxes(title_text='Date')
fig.update_yaxes(title_text='Rolling Correlation Coefficient')
fig.update_layout(plot_bgcolor='white')
