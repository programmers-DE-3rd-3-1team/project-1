...
# 데이터프레임 형식으로 저장
gold_kospi = pd.DataFrame(data, columns=['oil_price', 'date', 'gold_price'])

# 상관계수 출력 (전체 기간)
print(gold_kospi[['oil_price', 'gold_price']].corr(method='pearson'))

# 시간에 따른 상관계수 계산 (롤링 윈도우 사용)
window_size = 60  # 예: 30일 윈도우
rolling_corr = gold_kospi['oil_price'].rolling(window=window_size).corr(gold_kospi['gold_price'])

# 롤링 상관계수 그래프
fig = px.line(x=gold_kospi['date'], y=rolling_corr, title='Rolling Correlation: OIL vs GOLD')
